...
class StrongConsistency:
    ...
    def __init__(self, engine: EventEngine, network: Network, node_ids: List[str], timeout_ms: int = 2000):
        ...
        # State tracking for ongoing requests
        # req_id -> {"type": "read"|"write", "acks": set(node_id), "responses": list(msg), "callback": callback, "completed": bool}
    ...
    def write(self, key: str, value: Any, req_id: str, callback: Callable[[bool], None]) -> None:
        # For simplicity, version is driven by timestamp here, or coordinator can maintain a global version.
        # We will use clock current time as version.
    ...
    def receive(self, message: Message) -> None:
        ...
        if message.msg_type == MessageType.READ_RESP and req_state["type"] == "read":
            ...
            if len(req_state["acks"]) == len(self.node_ids):
                req_state["completed"] = True
                # Check if all returned values match. For strict strong consistency, they should all be the same version.
                # Here we just pick the one with the highest version, but if there's a partition and we got all acks,
                # they should all be identical anyway since we require ALL acks for writes.
    ...
    def _schedule_timeout(self, req_id: str):
        ...
        event = Event(
            ...
            event_type=EventType.CLIENT_REQUEST, # Just using a generic event type for timeouts
